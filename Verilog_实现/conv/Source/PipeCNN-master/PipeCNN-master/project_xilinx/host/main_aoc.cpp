...
#define WEIGHTS_FILE_SIZE 148
#define LAYER_NUM         1
#define CONV_NUM          5
const char *weight_file_path = "./data/data_test1/weights.dat";
const char *input_file_path  = "./data/data_test1/image.dat";
const char *ref_file_path    = "./data/data_test1/conv1.dat";
...
int main(int argc, char** argv)
{
	...
	for(unsigned i = 0; i < num_devices; ++i) {
		...
		for(unsigned j = 0; j < LAYER_NUM; ++j){
			...
			status = clEnqueueMigrateMemObjects(que_memRd[i], 1, &weights_buf[i*LAYER_NUM+j], 0, /* 0 means from host*/ 0, NULL, NULL);
	...
	for(unsigned i = 0; i < num_devices; ++i) {
		...
		for(unsigned char j = 0; j < LAYER_NUM; ++j){
			...
			if(layer_config[j][pool_on]==1){
				...
				status = clEnqueueTask(que_pool[i], knl_pool[i], 1, &memWr_event[i], &pool_event[i]);
